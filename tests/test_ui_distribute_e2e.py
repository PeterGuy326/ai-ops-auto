...
def test_full_flow_history_and_distribute(client):
    ...
    # 2) 历史回填（之前发的内容）
    r = client.post(
        f"/accounts/{aid}/import-published",
        json=[
            {
                "title": "去年爆款",
                "platform_post_id": "dy-1",
                "platform_url": "https://douyin.com/v/1",
            },
...
def test_ui_interactive_approve_distribute(client):
    ...
    # 点"分发"（勾选账号）
    r = client.post(
        f"/ui/articles/{art_id}/distribute", data={"account_ids": [aid]}, follow_redirects=False
    )
    assert r.status_code == 303
    recs = client.get(f"/accounts/{aid}/jobs").json()
...
def test_ui_library_filters(client):
    ...
    # 造不同类型 + 不同状态的素材
    v = client.post(
        "/articles", json={"topic_id": tid, "title": "视频草稿", "content_type": "video"}
    ).json()["id"]
    client.post(
        "/articles", json={"topic_id": tid, "title": "长文草稿", "content_type": "long_article"}
    )

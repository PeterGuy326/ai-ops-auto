...
闭环路径：
  publish 成功 → durable task ledger 记录 1h/24h/7d 三个采集窗口
  → 24h task 绑定快照回流后，调 evaluate_after_metrics(job_id, metric_id=...)
...
设计要点：
  ...
  - "近 N 次发布"窗口口径：按 PublishJob.finished_at desc 取最近 N 个 SUCCESS job，
    只比较每个 job 的 durable 24h 快照；真正的 legacy job 才回退 latest metric
  - 连续性从最新 job 向后计数，遇到健康或缺失证据立即中断
"""
...
from ..core.models import Account, Metrics, MetricsCollectionTask, PublishJob
...
def compute_baseline(session: Session, account_id: int, lookback_days: int = 7) -> dict:
    ...
    口径：已有持久任务的 job 只取精确 24h 窗口快照；有 24h task 但尚未有
    快照表示证据缺失，不允许用 manual/7d 值替代。只有完全没有 24h task 的历史
    job 才回退到最新 metric。
    ...
    for j in jobs:
        latest = _health_window_metric(session, j.id)
        if latest is None:
            continue
...
def _health_window_metric(session: Session, job_id: int) -> Metrics | None:
    """Return comparable 24h evidence, with fallback only for true legacy jobs."""
    task = (
        session.query(MetricsCollectionTask)
        .filter(
            MetricsCollectionTask.job_id == job_id,
            MetricsCollectionTask.window_seconds == 86400,
        )
        .one_or_none()
    )
    if task is not None:
        return (
            ...
            .filter(
                ...
                Metrics.collection_task_id == task.id,
            )
            .one_or_none()
        )
    return (
        ...
        .order_by(desc(Metrics.collected_at), desc(Metrics.id))
        .first()
    )
...
def evaluate_after_metrics(
    ...
    *,
    ...
    # Durable window callers pass the exact task-bound snapshot. Legacy/manual
    # callers retain the historical latest-row fallback.
    if metric_id is None:
        ...
        current_metric = (
            session.query(Metrics)
            .filter(Metrics.id == metric_id, Metrics.job_id == job_id)
            .one_or_none()
        )
    ...
    # 当前 low：回溯近 N 次 SUCCESS job 的同口径 24h metric，看是否连续低
    recent_count = _count_recent_low_views(
        session, account_id, baseline["views"], BAN_TRIGGER_WINDOW
    )
...
def _count_recent_low_views(
    ...
    """回溯该账号最近 `window` 个 SUCCESS job 的领先连续低曝光次数。

    口径：从最新 job 开始，低于 baseline_views * LOW_VIEW_RATIO 则累加；
    遇到第一条健康快照或缺失的持久 24h 证据立即中断。历史 job 仅在完全
    没有 24h task 时使用 latest-metric 回退。
    """
    ...
    jobs = (
        ...
        .order_by(desc(PublishJob.finished_at), desc(PublishJob.id))
        .limit(window)
        .all()
    )
    count = 0
